...
object Evaluation {
  ...
  - evaluation an expression eargerly
  ...
  val anotherComposedEvalution = for {
    value1 <- instantEval
    value2 <- delayedEval
  ...
  // "remember" a computed eval
  val dontRecompute = redoEval.memoize

  val tutorial = Eval
    .always { println("Step 1 ..."); "put the guitar on your lap"}
    .map { step1 => println ("Step 2"); s"$step1 then pur your left hand on the neck"}
