...
object Readers {
  ...
  // bootstrap
  val config = Configuration("sofia", "rockthejvm!", "localhost", 1234, 8, "sofia@rockthejvm.com")
  // cats Reader
  ...
  def getLastOderStatus(username: String): String = {
    // val usersLastOrderIdReader = dbReader.map(_.getLastOrderId(username)) // (dbcon => dbcon.getLastOrderId())
    // val usersLastOrderStatusReader = dbReader.map(_.getOrderStatus())

    val usersLastOrderIdReader = dbReader.map(_.getLastOrderId(username))
  ...
  def main(args: Array[String]): Unit = {
    println(getLastOderStatus("sofia"))
    println(emailUser("sofia", "sofia@rtjvm.com"))
  }
